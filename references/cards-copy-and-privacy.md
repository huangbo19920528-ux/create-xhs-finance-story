...
Use these defaults unless the user provides a brand system:

When the post uses `个人金融经验帖 / Trade25 Style`, also read [trade25-style.md](trade25-style.md) and follow its more specific question-led visual grammar.
...
4. Make `图片01/08 = 正文图1`, continuing one-to-one through `08/08 = 图8`; do not insert an extra cover. If a cover is needed, make it page 1 and let it answer the first question.
5. Run overflow checks where the editable format supports them.
6. Inspect every dense frame and the whole page at phone scale.
7. Fix alignment or padding before shrinking type.
8. Review all pages as a contact sheet for hierarchy and consistency.
9. Re-export after every copy change.
